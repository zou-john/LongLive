...
# ----------------------------- Argument parsing -----------------------------
parser = argparse.ArgumentParser("Interactive streaming causal inference")
...
for i, batch_data in tqdm(enumerate(dataloader), disable=(local_rank != 0)):
    ...
    for video_chunk, latents_chunk, is_final in pipeline.streaming_inference(
        ...
        # Write intermediate video chunk (accumulated)
        ...
        for seed_idx in range(config.num_samples):
            if config.save_with_index:
                ...
                output_path = os.path.join(config.output_folder, f"rank{stream_rank}-{short_name}-{seed_idx}_{model_type}.mp4")
            
            write_video(output_path, partial_video[seed_idx].to(torch.uint8), fps=16)
    ...
    print(f"[Streaming Interactive] Completed video generation with {num_generated_frames} frames")
